...
def encrypt(inputstring, key, n_rounds=16):
    ...
    if len(inputstring) % 64 != 0:
        inputstring += (64 - (len(inputstring) % 64)) * '0'
    # print("Input size = " + str(len(inputstring)))

    # Put each 64 bit chunk through DES
    while inputstring != "":
        binary_value = inputstring[:64]
